...
export default function Header({ activeUsersCount = 0 }: { activeUsersCount?: number }) {
  return (
    <header className="h-16 flex-shrink-0 bg-slate-900/30 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-4 md:px-6 sticky top-0 z-10">
      <div className="flex items-center gap-4">
        <button className="md:hidden p-2 text-slate-400 hover:text-slate-200 rounded-lg hover:bg-slate-800">
          <Menu size={20} />
        </button>
        <div className="hidden md:flex items-center gap-2 bg-slate-900/50 border border-slate-800 rounded-full px-4 py-1.5 focus-within:border-indigo-500/50 focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all">
